...
namespace PTOS {
	...
	template<typename T> struct Vector2 {
		...
		Vector2(const T(&data)[2]) {
			std::memcpy(&this->data, &data, sizeof(T) * 2);
		}

		Vector2(T* data, size_t len) {
			std::memcpy(&this->data, data, sizeof(T) * (len < 2 ? len : 2));
		}
	...
	template<typename T> struct Vector3 {
		...
		Vector3(const T(&data)[3]) {
			std::memcpy(&this->data, &data, sizeof(T) * 3);
		}

		Vector3(T* data, size_t len) {
			std::memcpy(&this->data, data, sizeof(T) * (len < 3 ? len : 3));
		}
		...
		inline T& w() { return data[2]; } //if using Vector3 as a Vector2 with a w component
		inline const T& x() const { return data[0]; }
		inline const T& y() const { return data[1]; }
		inline const T& z() const { return data[2]; }
		inline const T& w() const { return data[2]; }
		...
		inline Vector3<T> operator * (T m) const {
	...
	template<typename T> struct Vector4 {
		...
		Vector4(const T(&data)[4]) {
			std::memcpy(&this->data, &data, sizeof(T) * 4);
		}

		Vector4(T* data, size_t len) {
			std::memcpy(&this->data, data, sizeof(T) * (len < 4 ? len : 4));
		}
